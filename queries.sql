...
SELECT COUNT(*) AS number_not_escaped
FROM animals
...
SELECT vt.name,
    COALESCE(s.species, 'No Specialization') AS specialization
FROM vets vt
...
SELECT vt.name AS first_vet_visit
FROM vets vt
...
SELECT s.species AS recommended_specialty,
    COUNT(*) AS visit_count
FROM visits v
    JOIN animals a ON v.animal_id = a.id
    JOIN vets vt ON v.vet_id = vt.id
